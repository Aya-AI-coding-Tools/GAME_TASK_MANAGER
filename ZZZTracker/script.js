            ...
            <div class="task-item ${isCompleted ? 'completed' : ''}" data-id="${task.id}">
                ...
                <div class="task-reward ${polychromeClass}">
                    ${
                      task.polychrome > 0
                        ? `
                        <img src="https://act-upload.hoyoverse.com/event-ugc-hoyowiki/2025/01/22/59155606/b796be09d9ccafa4967bbbffec346ab9_7135762795127262940.png?x-oss-process=image%2Fformat%2Cwebp" alt="PC">
                        ${task.polychrome}
                    `
                        : task.reward
                    }
...
function initBattlepass() {
  ...
  levelInput.addEventListener('change', () => {
    state.battlepass.level = Math.min(
      60,
      Math.max(0, parseInt(levelInput.value) || 0),
    );
    levelInput.value = state.battlepass.level;
    updateBPGauge();
    saveState();
  ...
  expInput.addEventListener('change', () => {
    state.battlepass.exp = Math.min(
      800,
      Math.max(0, parseInt(expInput.value) || 0),
    );
    expInput.value = state.battlepass.exp;
    updateBPGauge();
    saveState();
...
function updateCountdowns() {
  ...
  document.getElementById('weekly-countdown').textContent = formatCountdown(
    weeklyReset - now,
  );
  ...
  document.getElementById('monthly-countdown').textContent = formatCountdown(
    monthlyReset - now,
  );
}
...
function checkVersionReset() {
  ...
  const daysSinceStart = Math.floor(
    (now - versionStart) / (1000 * 60 * 60 * 24),
  );
  ...
  if (daysSinceStart >= 42) {
    ...
    showNotificationBanner(
      '新バージョンが開始されました！バトルパスとポリクロームがリセットされました。',
    );
    updateVersionInfo();
    saveState();
  }
...
function updateVersionInfo() {
  ...
  const versionDiff = Math.floor(
    (versionStart - baseDate) / (42 * 24 * 60 * 60 * 1000),
  );
  const majorVersion = 2;
  const minorVersion = 5 + versionDiff;

  document.getElementById('current-version').textContent =
    `Ver ${majorVersion}.${minorVersion}`;
  document.getElementById('version-period').textContent =
    `${formatDate(versionStart)} - ${formatDate(versionEnd)}`;
}
...
function checkAndNotify() {
  ...
  if (dailyIncomplete) {
    sendBrowserNotification(
      'デイリータスク未完了',
      '今日のデイリータスクがまだ完了していません！',
    );
  }

  // 日曜日：ウィークリー通知
  if (dayOfWeek === 0) {
    const weeklyIncomplete = TASK_DATA.weekly.some(
      (task) => !state.tasks[task.id],
    );
    if (weeklyIncomplete) {
      sendBrowserNotification(
        'ウィークリータスク',
        '明日リセットです！ウィークリータスクを確認してください。',
      );
    }
  }
  ...
  if (state.settings.shiyuReset) {
    ...
    if (shiyuDate.toDateString() === tomorrow.toDateString()) {
      const biweeklyIncomplete = TASK_DATA.biweekly.some(
        (task) => !state.tasks[task.id],
      );
      if (biweeklyIncomplete) {
        sendBrowserNotification(
          '式輿防衛戦',
          '明日リセットです！式輿防衛戦を確認してください。',
        );
      }
    }
  }
  ...
  if (now.getDate() === lastDayOfMonth) {
    const monthlyIncomplete = TASK_DATA.monthly.some(
      (task) => !state.tasks[task.id],
    );
    if (monthlyIncomplete) {
      sendBrowserNotification(
        '毎月タスク',
        '明日リセットです！毎月タスクを確認してください。',
      );
    }
  }
}
...
function renderEvents() {
  ...
  if (state.events.length === 0) {
    container.innerHTML =
      '<p style="color: var(--text-secondary); text-align: center; padding: 20px;">手動追加されたイベントはありません</p>';
    return;
  }
...
function initSettings() {
  ...
  resetAllBtn.addEventListener('click', () => {
    if (
      confirm('本当に全データをリセットしますか？この操作は取り消せません。')
    ) {
